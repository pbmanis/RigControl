//
// RigControl: Arduino control of miscellaneous hardware that might be on 
// an electrophysiology rig. 
// Implemented so far:
// Temperature (Luke Campagnola)
// Valves, using the "Power Driver Shield". 3 valves at 12V, possibly use 12V on the 5V lines
// for more valves. 
// Lambda 10/2 - replaces the front panel keypad (whichi is dying) and sends serial commands to the 
// Lambda 10/2 to the filter choice and the shutter.
// 2011-2012 Paul B. Manis UNC Chapel Hill Otolaryngology/Head and Neck Surgery
// Supported by NIH/NIDCD DC000425, DC004551 and DC009809.
//
...
// These control the configuration - set to 1 if the device is avaiable,
// 0 otherwise.
...
#define VALVE2  5
#define VALVE3  6
#define VALVE4  6 //Sorry, no valve 4 in this system... due to limited digital IO. 
#define STATUS_LED 8
#define ON_BOARD_LED 13
#define TEMP_PWM_OUT 9 // note this goes on a 5-V line to the power driver shield. 

#define LOW 0
#define HIGH 1

int noutpins = 8;
int outpins[] = {3,5,6,9,8,10,11,12};
...
int bldur = 20; // blink duration
int blint = 40; // interval
void blpattern(int n, int m) {
  nblink(n);
  delay(blint*3); // word spacing
  nblink(m);
}

void nblink(int n) {
  int i;
  for (i = 0; i < n; i++) { // blink n times, with duration dur and interval as specified
    blink();
    delay(blint);
  }
}

void blink() { // generate one blink of duration bldur
...
void setup() {
  Serial.begin(115200); // use fast communication. 
  setoutputs(); // make sure digital outputs are configured
